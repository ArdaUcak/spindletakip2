...
class MainApp:
    def __init__(self):
        ...
        self.spindle_manager = DataManager(
            "spindle_data.csv",
            ["id", "Makine No", "Spindle Ref", "Çalışma Saati", "Son Güncelleme"],
        )
        self.yedek_manager = DataManager(
            "yedek_data.csv",
            ["id", "Referans No", "Açıklama", "Tamirde mi", "Gönderilen Tarih", "Dönüş Tarihi", "Son Güncelleme"],
        )
    ...
    def _build_spindle_tab(self, parent):
        ...
        labels = ["Makine No", "Spindle Ref", "Çalışma Saati", "Son Güncelleme"]
        ...
        ttk.Button(btn_frame, text="Ekle", command=self._add_spindle).pack(side=tk.LEFT, padx=5, pady=5)
        ttk.Button(btn_frame, text="Sil", command=self._delete_spindle).pack(side=tk.LEFT, padx=5, pady=5)
        ttk.Button(btn_frame, text="Güncelle", command=self._update_spindle).pack(side=tk.LEFT, padx=5, pady=5)
        ttk.Button(btn_frame, text="Ara", command=self._search_spindle).pack(side=tk.LEFT, padx=5, pady=5)

        tree_frame = ttk.Frame(parent)
        tree_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        columns = ["id", "Makine No", "Spindle Ref", "Çalışma Saati", "Son Güncelleme"]
    ...
    def _build_yedek_tab(self, parent):
        ...
        self.yedek_entries["Gönderilen Tarih"].insert(0, today)
        self.yedek_entries["Dönüş Tarihi"].insert(0, today)
        self.yedek_entries["Son Güncelleme"].insert(0, today)

        btn_frame = ttk.Frame(parent)
        btn_frame.pack(fill=tk.X, padx=10)
        ttk.Button(btn_frame, text="Ekle", command=self._add_yedek).pack(side=tk.LEFT, padx=5, pady=5)
        ttk.Button(btn_frame, text="Sil", command=self._delete_yedek).pack(side=tk.LEFT, padx=5, pady=5)
        ttk.Button(btn_frame, text="Güncelle", command=self._update_yedek).pack(side=tk.LEFT, padx=5, pady=5)
        ttk.Button(btn_frame, text="Ara", command=self._search_yedek).pack(side=tk.LEFT, padx=5, pady=5)
    ...
    def _add_spindle(self):
        ...
        if not values["Makine No"] or not values["Spindle Ref"]:
            messagebox.showerror("Hata", "Makine No ve Spindle Ref zorunludur.")
            return
    ...
    def _update_spindle(self):
        selected = self.spindle_tree.selection()
        if not selected:
            messagebox.showerror("Hata", "Güncellemek için bir kayıt seçin.")
            return
        record_id = self.spindle_tree.item(selected[0], "values")[0]
        values = self._get_entry_values(self.spindle_entries)
    ...
    def _search_spindle(self):
        filters = {
            "Makine No": self.spindle_entries["Makine No"].get(),
            "Spindle Ref": self.spindle_entries["Spindle Ref"].get(),
        }
        results = self.spindle_manager.search(**filters)
        self._refresh_spindle_tree(results)

    def _add_yedek(self):
        values = self._get_entry_values(self.yedek_entries)
        if not values["Referans No"]:
            messagebox.showerror("Hata", "Referans No zorunludur.")
            return
    ...
    def _search_yedek(self):
        filters = {
            "Referans No": self.yedek_entries["Referans No"].get(),
            "Açıklama": self.yedek_entries["Açıklama"].get(),
        }
        results = self.yedek_manager.search(**filters)
    ...
    def _export_data(self):
        ...
        with open(export_path, "w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow(["Makine No", "Ref", "Saat", "Son Güncelleme"])
            ...
            writer.writerow(["Ref No", "Açıklama", "Tamirde", "Gönderildi", "Dönen", "Son Güncelleme"])
            for row in yedek_rows:
                writer.writerow([
                    row.get("Referans No", ""),
                    row.get("Açıklama", ""),
                    row.get("Tamirde mi", ""),
                    row.get("Gönderilen Tarih", ""),
                    row.get("Dönüş Tarihi", ""),
                    row.get("Son Güncelleme", ""),
                ])
